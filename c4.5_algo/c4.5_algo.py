...
def nominalize(dataset, attribute):
    sortedValues = np.sort(dataset[attribute])
...
if __name__ == '__main__':
    ...
    attributesToNominalize = ['Temp.', 'Humidity']

    for attr in attributesToNominalize:
        set1 = set(attributesToNominalize)
        set2 = set([attr])
        attributesToDrop = list(set1.difference(set2))
        new_dt = (dataset.drop(labels=attributesToDrop, axis=1))
        nominalize(new_dt, attr)


    '''now that the data has been nominalized we can simply feed this data into the ID3 algorithm'''
